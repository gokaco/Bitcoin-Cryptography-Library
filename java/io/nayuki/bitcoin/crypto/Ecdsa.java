...
public final class Ecdsa {
	...
	public static boolean sign(@Unsigned int[] privateKey, Sha256Hash msgHash, @Unsigned int[] nonce, int[] outR, int[] outS) {
		...
		@Unsigned int[] val = new int[2 * NUM_WORDS + CurvePointMath.POINT_WORDS + CurvePointMath.MULTIPLY_TEMP_WORDS];  // Temporary scratch space for all values
		...
		@Unsigned int rOff = pOff + CurvePointMath.XCOORD;  // Uint256, aliasing p.x
		...
		@Unsigned int sOff = pOff + CurvePointMath.YCOORD;  // Uint256, reuses space
		@Unsigned int zOff = pOff + CurvePointMath.ZCOORD;  // Uint256, reuses space
		...
		@Unsigned int carry = Int256Math.uintAdd(val, sOff, zOff, 1, sOff);
		Int256Math.uintSubtract(val, sOff, orderOff, carry | (Int256Math.lessThan(val, sOff, orderOff) ^ 1), sOff);
		
		@Unsigned int kInvOff = zOff;  // Uint256, reuses space
		...
		@Unsigned int negSOff = zOff;  // Uint256, reuses space
	...
	public static boolean signWithHmacNonce(@Unsigned int[] privateKey, Sha256Hash msgHash, int[] outR, int[] outS) {
		Objects.requireNonNull(privateKey);
		Objects.requireNonNull(msgHash);
		Objects.requireNonNull(outR);
		Objects.requireNonNull(outS);
		...
		@Unsigned byte[] privkeyBytes = Int256Math.uintToBytes(privateKey, 0);
		@Unsigned byte[] msghashBytes = msgHash.toBytes();
		@Unsigned byte[] hmac = Sha256.getHmac(privkeyBytes, msghashBytes).toBytes();
		@Unsigned int[] nonce = new int[Int256Math.NUM_WORDS];
	...
	public static boolean verify(@Unsigned int[] publicKey, Sha256Hash msgHash, @Unsigned int[] r, @Unsigned int[] s) {
		...
		@Unsigned int[] val = new int[9 * NUM_WORDS + CurvePointMath.MULTIPLY_TEMP_WORDS];
		@Unsigned int tempOff  = 9 * NUM_WORDS;
		@Unsigned int orderOff = 0 * NUM_WORDS;  // Uint256
		System.arraycopy(CurvePointMath.ORDER, 0, val, orderOff, NUM_WORDS);
		
		@Unsigned int rOff = 1 * NUM_WORDS;  // Uint256
		@Unsigned int sOff = 2 * NUM_WORDS;  // Uint256
		...
		@Unsigned int qOff   = 3 * NUM_WORDS;  // CurvePoint
		@Unsigned int oneOff = 6 * NUM_WORDS;  // Uint256
		...
		@Unsigned int wOff = 2 * NUM_WORDS;  // Uint256, reuses space
		Int256Math.reciprocal(val, sOff, orderOff, wOff, tempOff);
		
		@Unsigned int u1Off = 3 * NUM_WORDS;  // Uint256, reuses space
		@Unsigned int u2Off = 2 * NUM_WORDS;  // Uint256, reuses space
		@Unsigned int zOff  = 4 * NUM_WORDS;  // Uint256, reuses space
		Int256Math.bytesToUint(msgHash.toBytes(), val, zOff);
		...
		@Unsigned int pOff = 6 * NUM_WORDS;  // CurvePoint, reuses space
		System.arraycopy(CurvePointMath.BASE_POINT, 0, val, pOff, CurvePointMath.POINT_WORDS);
		...
		CurvePointMath.normalize(val, pOff, tempOff);
		@Unsigned int reduce = Int256Math.lessThan(val, pOff + CurvePointMath.XCOORD, orderOff) ^ 1;
	...
	private static void multiplyModOrder(@Unsigned int[] x, @Unsigned int xOff, @Unsigned int[] y, @Unsigned int yOff, @Unsigned int tempOff) {
		/* 
		 ...
		 * x = z
		 */
		@Unsigned int modOff = tempOff + 0 * NUM_WORDS;
		@Unsigned int zOff   = tempOff + 1 * NUM_WORDS;
		...
		for (@Unsigned int i = Int256Math.NUM_WORDS * 32 - 1; i >= 0; i--) {
			// Multiply by 2
			@Unsigned int c = Int256Math.uintShiftLeft1(x, zOff, zOff);
			Int256Math.uintSubtract(x, zOff, modOff, c | (Int256Math.lessThan(x, zOff, modOff) ^ 1), zOff);
			// Conditionally add x
			@Unsigned int enable = (y[yOff + (i >>> 5)] >>> (i & 31)) & 1;
